...
void finishHists_M1M3(){
    TFile *fileIn = TFile::Open("hist_GGM_M1M3.root");

    TFile *fileOut = TFile::Open("output_M1M3.root","RECREATE");
    fileOut->cd();
    ...
    ifstream M1M2_xsec ("M1M3_xsec.txt");
    ...
    for(int i = 1; i <= xbinnum3;i++){
	  ...
	  //Notice that the bin histogram starts counting at 1 but the array starts at 0. So the first bin (i=1) is given by xbins3[1] -25 = 75-25 = 50
