...
TH1F *h_ee_DiEmPt_ratio = (TH1F*)h_G_data->Clone("h_ee_DiEmPt_ratio");
 h_ee_DiEmPt_ratio->SetLineColor(kBlack);
 h_ee_DiEmPt_ratio->SetMinimum(0.0);
 h_ee_DiEmPt_ratio->SetMaximum(4.5);
 h_ee_DiEmPt_ratio->SetStats(0);
 h_ee_DiEmPt_ratio->Divide(h_E_data);
 //h_ee_DiEmPt_ratio->Divide(h_E_BG);
 h_ee_DiEmPt_ratio->SetMarkerColor(1);
 h_ee_DiEmPt_ratio->SetMarkerSize(1);
 h_ee_DiEmPt_ratio->SetMarkerStyle(20);

 TFile outputfile("EE_DiEmPtRatio.root","RECREATE");

 h_ee_DiEmPt_ratio->Write();
 outputfile.Close();


cout<<"\nwithout signal for data electrons;"<<endl;
 for(int i=1; i<28; i++){
   cout<<h_ee_DiEmPt_ratio->GetBinContent(i)<<", ";
 }


  cout<<"\nRatio error for data electrons;"<<endl;
 for(int i=1; i<28; i++){
   cout<<(h_ee_DiEmPt_ratio->GetBinError(i))<<", ";
 }
