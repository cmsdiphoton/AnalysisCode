...
// Wed Mar 15 15:15:13 2017 by ROOT version 5.34/36
// from TTree EventTree/Event data (tag V08_00_24_00)
// found on file: root://eoscms//eos/cms/store/group/phys_higgs/itopsisg/lisa/crab/crab3/Spring16_80X/data/miniAOD_data16/allData_2016_MiniAOD.root
//////////////////////////////////////////////////////////

#ifndef T5gg_analyzer_h
#define T5gg_analyzer_h
#include <fstream>
#include <TROOT.h>
#include <TChain.h>
#include <TFile.h>
...
class T5gg_analyzer : public TSelector {
public :
   ...
   T5gg_analyzer(TTree * /*tree*/ =0) : fChain(0) { }
   virtual ~T5gg_analyzer() { }
 ...
 virtual TLorentzVector       p4vec(float pt, float phi, float pz);
 static  bool		     P4SortCondition(const TLorentzVector& p1, const TLorentzVector& p2) {return (p1.Pt() > p2.Pt());}
 virtual std::pair<float,int> minDR_ValueAndIndex(const TLorentzVector lv, const vector<TLorentzVector> v);
 virtual int		     IsGoodPhoton(int iPho, int iSel, int verbose);
 virtual void               IncludeAJson(std::string jsonfile);
 static bool                 isInJson(Int_t run, Int_t lumi);


   ClassDef(T5gg_analyzer,0);
};

#endif

#ifdef T5gg_analyzer_cxx
void T5gg_analyzer::Init(TTree *tree)
{
   ...
   myfile.open("eeEvt_Met50to60.txt");
   phoPrescale = 0;
   phoE = 0;
   phoEt = 0;
   phoEta = 0;
